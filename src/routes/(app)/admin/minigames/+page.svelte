...
<div class="space-y-6">
	...
	<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
		<div class="glass rounded-xl p-4 flex items-center gap-3">
			<div class="w-10 h-10 rounded-lg bg-[var(--magic-purple)]/20 flex items-center justify-center">
				<Gamepad2 size={20} class="text-[var(--magic-purple)]" />
			</div>
			<div>
				<p class="text-2xl font-bold text-[var(--color-text-primary)]">{$miniGameStats.total}</p>
				<p class="text-xs text-[var(--color-text-muted)]">Total</p>
			</div>
		</div>
		<div class="glass rounded-xl p-4 flex items-center gap-3">
			<div class="w-10 h-10 rounded-lg bg-[var(--magic-turquoise)]/20 flex items-center justify-center">
				<Zap size={20} class="text-[var(--magic-turquoise)]" />
			</div>
			<div>
				<p class="text-2xl font-bold text-[var(--magic-turquoise)]">{$miniGameStats.actifs}</p>
				<p class="text-xs text-[var(--color-text-muted)]">Actifs</p>
			</div>
		</div>
		<div class="glass rounded-xl p-4 flex items-center gap-3">
			<div class="w-10 h-10 rounded-lg bg-[var(--magic-orange)]/20 flex items-center justify-center">
				<Edit size={20} class="text-[var(--magic-orange)]" />
			</div>
			<div>
				<p class="text-2xl font-bold text-[var(--magic-orange)]">{$miniGameStats.brouillons}</p>
				<p class="text-xs text-[var(--color-text-muted)]">Brouillons</p>
			</div>
		</div>
		<div class="glass rounded-xl p-4 flex items-center gap-3">
			<div class="w-10 h-10 rounded-lg bg-[var(--color-text-muted)]/20 flex items-center justify-center">
				<Trash2 size={20} class="text-[var(--color-text-muted)]" />
			</div>
			<div>
				<p class="text-2xl font-bold text-[var(--color-text-muted)]">{$miniGameStats.archives}</p>
				<p class="text-xs text-[var(--color-text-muted)]">Archivés</p>
			</div>
		</div>
	</div>
		...
		<div class="flex flex-wrap gap-4">
			...
			<div class="relative flex-1 min-w-[200px]">
				...
				<input 
					...
					class="w-full pl-10 pr-4 py-2 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-lg text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--magic-turquoise)]"
				/>
			</div>
			...
			<select 
				...
				class="px-4 py-2 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-lg text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--magic-turquoise)]"
			>
	...
	{:else if filteredMiniGames.length === 0}
		<div class="glass rounded-xl p-12 text-center">
			<Gamepad2 size={48} class="mx-auto text-[var(--color-text-muted)] mb-4" />
	...
	{:else}
		<div class="grid gap-4">
			{#each filteredMiniGames as game (game.id)}
				...
				<div class="glass rounded-xl p-4 hover:border-[var(--magic-turquoise)]/30 transition-colors group">
					<div class="flex items-center gap-4">
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2 mb-1">
								<h3 class="font-semibold text-[var(--color-text-primary)] truncate group-hover:text-[var(--magic-turquoise)] transition-colors">
									{game.nom}
								</h3>
								<span 
							...
							<div class="flex items-center gap-4 mt-2 text-xs text-[var(--color-text-muted)]">
								...
								{#if parentModule}
									<span class="flex items-center gap-1 text-[var(--magic-purple)]">
										<Link size={12} />
						...
						<div class="relative">
							...
							>
								<MoreVertical size={18} class="text-[var(--color-text-muted)]" />
							</button>

							{#if openMenuId === game.id}
								<div 
									class="absolute right-0 top-full mt-1 w-40 glass rounded-lg border border-[var(--color-border)] shadow-lg z-50 py-1"
									onclick={(e) => e.stopPropagation()}
								>
									<a 
										href="/admin/minigames/{game.id}"
										class="flex items-center gap-2 px-3 py-2 text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] hover:text-[var(--color-text-primary)]"
									>
										<Edit size={14} />
										Modifier
									</a>
